...
async def main():
    ...
    async with AsyncSessionLocal() as session:
        ...
        # -----------------------------------------------------
        # Test Retriever DIRECTLY - FIRST TIME
        # -----------------------------------------------------

        print("\nCalling RelevantMessageRetriever - FIRST TIME...\n")
        ...
        print(
            "\n========== RELEVANT MESSAGES - FIRST RETRIEVAL ==========\n"
        )
        ...
        # -----------------------------------------------------
        # Add ONE MORE message AFTER first retrieval
        # -----------------------------------------------------

        print(
            "Adding a NEW message to PostgreSQL...\n"
        )
        ...
        new_message = ConversationMessage(
            ...
            role="user",
            content=(
                "This is message number 11 added after the first "
                "retrieval. FastAPI is used to build the AI service "
                "API, while PostgreSQL is used as the source of truth "
                "for persistent conversation data."
            ),
            created_at=datetime.now(timezone.utc),
            sequence_number=11,
        )

        await repository.save_message(new_message)

        print(
            "Message 11 successfully saved to PostgreSQL.\n"
        )
        ...
        print(
            "Calling RelevantMessageRetriever - SECOND TIME...\n"
        )

        relevant_messages_again = (
            await relevant_message_retriever.retrieve(
                conversation_id=conversation_id,
                query=current_message,
                limit=3,
        ...
        print(
            "\n========== RELEVANT MESSAGES - SECOND RETRIEVAL ==========\n"
        )

        print(
            f"Number of relevant messages: "
            f"{len(relevant_messages_again)}"
        )

        for result in relevant_messages_again:
        ...
        # -----------------------------------------------------
        # Get LATEST history from PostgreSQL
        # -----------------------------------------------------

        print(
            "Loading latest conversation history from PostgreSQL...\n"
        )

        latest_history = await repository.get_history(
            conversation_id
        )

        print(
            f"Latest history message count: "
            f"{len(latest_history)}\n"
        )
